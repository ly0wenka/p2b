...
public class RandomAI : AbstractInputController
{
    ...
    public override void DoUpdate()
    {
        if (this.inputReferences != null)
        {
            ...
            //---------------------------------------------------------------------------------------------------------
            this.inputs.Clear();
            if (currentTime - this.timeLastDecision >= MainScript.config.aiOptions.inputFrequency)
            {
                this.timeLastDecision = currentTime;
    ...
    {
        ControlsScript self = MainScript.GetControlsScript(this.player);
        if (self != null)
        {
            ...
            if (opponent != null)
            {
                ...
                foreach (AIDistanceBehaviour thisBehaviour in MainScript.config.aiOptions.distanceBehaviour)
                {
                    if (thisBehaviour != null)
                    {
                        if (distance >= thisBehaviour.proximityRangeBegins &&
                            distance <= thisBehaviour.proximityRangeEnds)
                        {
                ...
                {
                    float axis = 0f;
                    if (MainScript.config.aiOptions.moveWhenEnemyIsDown || !isOpponentDown)
                    {
                        axis =
                            Mathf.Sign(dx)
                            *
                ...
                {
                    if (!MainScript.config.aiOptions.attackWhenEnemyIsDown && isOpponentDown)
                    {
                        return InputEvents.Default;
                    }
