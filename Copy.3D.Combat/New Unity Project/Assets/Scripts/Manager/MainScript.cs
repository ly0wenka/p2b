...
public class MainScript : MonoBehaviour, MainScriptInterface
{
    ...
    public delegate void BodyVisibilityHandler(MoveInfo move, CharacterInfo player,
    ...
    public delegate void ParticleEffectsHandler(MoveInfo move, CharacterInfo player,
    ...
    {
        get { return MainScript._multiplayerMode; }
        set
        {
    ...
    {
        get { return _fixedDeltaTime * timeScale; }
    ...
    public static GameObject SpawnGameObject(GameObject gameObject, Vector3 position, Quaternion rotation,
        long? destroyTimer = null)
    {
        ...
        MainScript.instantiatedObjects.Add(new InstantiatedGameObject(goInstance, mrFusion, MainScript.currentFrame,
            MainScript.currentFrame + destroyTimer));
    ...
    public static void DestroyGameObject(GameObject gameObject, long? destroyTimer = null)
    {
        for (int i = 0; i < MainScript.instantiatedObjects.Count; ++i)
        {
            ...
            {
                MainScript.instantiatedObjects[i].destructionFrame =
                    destroyTimer == null ? MainScript.currentFrame : destroyTimer;
                break;
            }
        }
    }
    ...
    public static CharacterStory GetCharacterStory(CharacterInfo character)
    {
        if (!MainScript.config.storyMode.useSameStoryForAllCharacters)
        {
            ...
            {
                if (MainScript.config.characters[i] == character &&
                    storyMode.selectableCharactersInStoryMode.Contains(i))
                {
    ...
    public static void ShowScreen(CombatScreen screen, Action nextScreenAction = null)
    {
        if (screen != null)
        {
            ...
            MainScript.currentScreen.transform.SetParent(MainScript.canvas != null ? MainScript.canvas.transform : null,
                false);
    ...
    public static void StartBluetoothGameScreen(float fadeTime)
    {
        if (MainScript.currentScreen.hasFadeOut)
        {
            ...
            MainScript.DelayLocalAction(() =>
            {
                MainScript.eventSystem.enabled = true;
    ...
    {
        MainScript.StartStoryModeConversationAfterBattleScreen(conversationScreen,
            (float) MainScript.config.gameGUI.screenFadeDuration);
    }
    ...
    {
        MainScript.StartStoryModeConversationBeforeBattleScreen(conversationScreen,
            (float) MainScript.config.gameGUI.screenFadeDuration);
    }
    ...
    {
        MainScript.storyMode.defeatedOpponents.Add(MainScript.storyMode.currentBattleInformation
            .opponentCharacterIndex);
        MainScript.StartStoryModeConversationAfterBattleScreen(
            MainScript.storyMode.currentBattleInformation.conversationAfterBattle, fadeTime);
    }
    ...
    public static void FireBodyVisibilityChange(MoveInfo move, CharacterInfo player,
        BodyPartVisibilityChange bodyPartVisibilityChange, HitBox hitBox)
    {
        if (MainScript.OnBodyVisibilityChange != null)
            MainScript.OnBodyVisibilityChange(move, player, bodyPartVisibilityChange, hitBox);
    }
    ...
    public static void HostBluetoothGame()
    {
        if (MainScript.isNetworkAddonInstalled)
        {
            ...
            MainScript.multiplayerAPI.CreateMatch(
                new MultiplayerAPI.MatchCreationRequest(MainScript.config.networkOptions.port, null, 1, false, null));
        }
    }
    ...
    protected void Awake()
    {
...
#else
        MainScript.isNetworkAddonInstalled = MainScript.IsInstalled("UnetHighLevelMultiplayerAPI") &&
                                             MainScript.config.networkOptions.networkService != NetworkService.Disabled;
        MainScript.isPhotonInstalled = MainScript.IsInstalled("PhotonMultiplayerAPI") &&
                                       MainScript.config.networkOptions.networkService != NetworkService.Disabled;
        MainScript.isBluetoothAddonInstalled = MainScript.IsInstalled("BluetoothMultiplayerAPI") &&
                                               MainScript.config.networkOptions.networkService !=
        ...
        if (MainScript.isControlFreakInstalled &&
            MainScript.config.inputOptions.inputManagerType == InputManagerType.ControlFreak)
        {
            if (MainScript.isControlFreak2Installed && (MainScript.config.inputOptions.controlFreak2Prefab != null))
            {
                ...
                MainScript.controlFreakPrefab =
                    (GameObject) Instantiate(MainScript.config.inputOptions.controlFreak2Prefab.gameObject);
                MainScript.touchControllerBridge = (MainScript.controlFreakPrefab != null)
                    ? MainScript.controlFreakPrefab.GetComponent<InputTouchControllerBridge>()
                    : null;
            ...
            else if (MainScript.isControlFreak1Installed && (MainScript.config.inputOptions.controlFreakPrefab != null))
            {
                ...
                MainScript.controlFreakPrefab =
                    (GameObject) Instantiate(MainScript.config.inputOptions.controlFreakPrefab);
            }
        }
        ...
        if (MainScript.isNetworkAddonInstalled)
        {
            ...
            MainScript.lanMultiplayerAPI =
                networkManager.AddComponent(MainScript.SearchClass("UnetLanMultiplayerAPI")) as MultiplayerAPI;
            MainScript.lanMultiplayerAPI.Initialize(uuid);

            if (MainScript.config.networkOptions.networkService == NetworkService.Unity)
            {
                MainScript.onlineMultiplayerAPI =
                    networkManager.AddComponent(MainScript.SearchClass("UnetOnlineMultiplayerAPI")) as MultiplayerAPI;
            }
            else if (MainScript.config.networkOptions.networkService == NetworkService.Photon &&
                     MainScript.isPhotonInstalled)
            {
                MainScript.onlineMultiplayerAPI =
                    networkManager.AddComponent(MainScript.SearchClass("PhotonMultiplayerAPI")) as MultiplayerAPI;
            }
            else if (MainScript.config.networkOptions.networkService == NetworkService.Photon &&
                     !MainScript.isPhotonInstalled)
            {
                Debug.LogError(
                    "You need 'Photon Unity Networking' installed in order to use Photon as a Network Service.");
            }
            ...
            {
                MainScript.bluetoothMultiplayerAPI =
                    networkManager.AddComponent(MainScript.SearchClass("BluetoothMultiplayerAPI")) as MultiplayerAPI;
            }
            else
            {
        ...
        MainScript.fluxCapacitor =
            new FluxCapacitor(MainScript.currentFrame, MainScript.config.networkOptions.maxBufferSize);
    ...
    protected void Update()
    {
        ...
        if (MainScript.fluxCapacitor != null && MainScript.config.debugOptions.stateTrackerTest)
        {
            ...
            if (MainScript.fluxCapacitor.savedState != null && Input.GetKeyDown(KeyCode.F3))
            {
                // Load State
                ...
                MainScript.fluxCapacitor.PlayerManager.Initialize(
                    MainScript.fluxCapacitor.savedState.Value.networkFrame);
    ...
    {
        get { return MainScript.multiplayerAPI.IsConnected() && MainScript.multiplayerAPI.Connections > 0; }
    }
    ...
    private static void _StartBluetoothGameScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Bluetooth Game Screen not found! Make sure you have set the prefab correctly in the Global Editor");
        }
    ...
    private static void _StartCharacterSelectionScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Character Selection Screen not found! Make sure you have set the prefab correctly in the Global Editor");
        }
        else
        {
    ...
    private static void _StartMainMenuScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Main Menu Screen not found! Make sure you have set the prefab correctly in the Global Editor");
        }
        else
        {
    ...
    private static void _StartStageSelectionScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Stage Selection Screen not found! Make sure you have set the prefab correctly in the Global Editor");
        }
        else
        {
    ...
    private static void _StartConnectionLostScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Connection Lost Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartMainMenuScreen(fadeTime);
        }
        else if (MainScript.isNetworkAddonInstalled)
        {
    ...
    private static void _StartGame(float fadeTime)
    {
        ...
        if (MainScript.config.stagePrefabStorage == StorageMode.Legacy)
        {
            ...
            {
                Debug.LogError(
                    "Stage prefab not found! Make sure you have set the prefab correctly in the Global Editor.");
            }
        }
        else
        {
            ...
            {
                Debug.LogError(
                    "Stage prefab not found! Make sure the prefab is correctly located under the Resources folder and the path is written correctly.");
            }
        }
        ...
        if (MainScript.isControlFreak2Installed &&
            MainScript.p1ControlsScript.myInfo.customControls.overrideControlFreak &&
            MainScript.p1ControlsScript.myInfo.customControls.controlFreak2Prefab != null)
        {
            MainScript.controlFreakPrefab =
                (GameObject) Instantiate(MainScript.p1ControlsScript.myInfo.customControls.controlFreak2Prefab
                    .gameObject);
            MainScript.touchControllerBridge = (MainScript.controlFreakPrefab != null)
                ? MainScript.controlFreakPrefab.GetComponent<InputTouchControllerBridge>()
                : null;
        ...
        if (MainScript.config.player1Character.name == MainScript.config.player2Character.name)
        {
            if (MainScript.config.player2Character.alternativeCostumes.Length > 0)
            {
                ...
                if (MainScript.config.player2Character.alternativeCostumes[0].characterPrefabStorage ==
                    StorageMode.Legacy)
                {
                    MainScript.p2ControlsScript.myInfo.characterPrefab =
                        MainScript.config.player2Character.alternativeCostumes[0].prefab;
                }
                else
                {
                    MainScript.p2ControlsScript.myInfo.characterPrefab =
                        Resources.Load<GameObject>(MainScript.config.player2Character.alternativeCostumes[0]
                            .prefabResourcePath);
                }
            }
        }
        ...
        MainScript.debugger1 = MainScript.DebuggerText("Debugger1", "",
            new Vector2(-Screen.width + 50, Screen.height - 180), TextAnchor.UpperLeft);
        MainScript.debugger2 = MainScript.DebuggerText("Debugger2", "",
            new Vector2(Screen.width - 50, Screen.height - 180), TextAnchor.UpperRight);
    ...
    private static void _StartHostGameScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Host Game Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartMainMenuScreen(fadeTime);
        }
        else if (MainScript.isNetworkAddonInstalled)
        {
    ...
    private static void _StartJoinGameScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Join To Game Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartMainMenuScreen(fadeTime);
        }
        else if (MainScript.isNetworkAddonInstalled)
        {
    ...
    private static void _StartLoadingBattleScreen(float fadeTime)
    {
        ...
        {
            Debug.Log(
                "Loading Battle Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartGame((float) MainScript.config.gameGUI.gameFadeDuration);
        }
        else
        {
    ...
    private static void _StartSearchMatchScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Random Match Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartMainMenuScreen(fadeTime);
        }
        else if (MainScript.isNetworkAddonInstalled)
        {
    ...
    private static void _StartNetworkGameScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Network Game Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartMainMenuScreen(fadeTime);
        }
        else if (MainScript.isNetworkAddonInstalled)
        {
    ...
    private static void _StartOptionsScreen(float fadeTime)
    {
        ...
        {
            Debug.LogError(
                "Options Screen not found! Make sure you have set the prefab correctly in the Global Editor");
        }
        else
        {
    ...
    public static void _StartStoryModeBattle(float fadeTime)
    {
        ...
        if (MainScript.player1WonLastBattle)
        {
            ...
            else if (MainScript.storyMode.currentGroup >= 0 && MainScript.storyMode.currentGroup <
                MainScript.storyMode.characterStory.fightsGroups.Length)
            {
                // Otherwise, check if there are more remaining battles in the current group
                FightsGroup currentGroup =
                    MainScript.storyMode.characterStory.fightsGroups[MainScript.storyMode.currentGroup];
            ...
            )
            {
                ...
                if (currentGroup.mode == FightsGroupMode.FightAgainstAllOpponentsInTheGroupInTheDefinedOrder)
                {
                    ...
                    if (MainScript.storyMode.canFightAgainstHimself ||
                        !character.characterName.Equals(opponent.characterName))
                    {
        ...
        if (MainScript.storyMode.currentBattleInformation != null)
        {
            ...
            if (MainScript.player1WonLastBattle)
            {
                MainScript.lastStageIndex = UnityEngine.Random.Range(0,
                    MainScript.storyMode.currentBattleInformation.possibleStagesIndexes.Count);
            }

            MainScript.SetStage(MainScript.config.stages[
                MainScript.storyMode.currentBattleInformation.possibleStagesIndexes[MainScript.lastStageIndex]]);

            // Finally, check if we should display any "Conversation Screen" before the battle
            MainScript._StartStoryModeConversationBeforeBattleScreen(
                MainScript.storyMode.currentBattleInformation.conversationBeforeBattle, fadeTime);
        }
        else
        {
    ...
    private static void _StartStoryModeCongratulationsScreen(float fadeTime)
    {
        ...
        {
            Debug.Log(
                "Congratulations Screen not found! Make sure you have set the prefab correctly in the Global Editor");
            MainScript._StartStoryModeEndingScreen(fadeTime);
        }
        else
        {
            MainScript.ShowScreen(MainScript.config.gameGUI.storyModeCongratulationsScreen,
                delegate() { MainScript.StartStoryModeEndingScreen(fadeTime); });
    ...
    private static void _StartStoryModeEndingScreen(float fadeTime)
    {
        ...
        {
            MainScript.ShowScreen(MainScript.storyMode.characterStory.ending,
                delegate() { MainScript.StartCreditsScreen(fadeTime); });
    ...
    private static void _StartStoryModeGameOverScreen(float fadeTime)
    {
        ...
        {
            MainScript.ShowScreen(MainScript.config.gameGUI.storyModeGameOverScreen,
                delegate() { MainScript.StartMainMenuScreen(fadeTime); });
    ...
    private static void _StartStoryModeOpeningScreen(float fadeTime)
    {
        ...
        {
            MainScript.ShowScreen(MainScript.storyMode.characterStory.opening,
                delegate() { MainScript.StartStoryModeBattle(fadeTime); });
    ...
    private static void _StartVersusModeAfterBattleScreen(float fadeTime)
    {
        ...
        {
            Debug.Log(
                "Versus Mode \"After Battle\" Screen not found! Make sure you have set the prefab correctly in the Global Editor");
