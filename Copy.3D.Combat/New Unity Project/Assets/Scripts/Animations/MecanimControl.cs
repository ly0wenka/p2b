...
public class MecanimControl : MonoBehaviour
{
    ...
    {
        get { return this.animator.runtimeAnimatorController; }
        set { this.animator.runtimeAnimatorController = value; }
    }

    public bool applyRootMotion
    {
        get { return this.animator.applyRootMotion; }
        set { this.animator.applyRootMotion = value; }
    }
    ...
    public void DoFixedUpdate()
    {
        ...
        if (overrideAnimatorUpdate)
        {
            ...
            animator.Update((float) MainScript.fixedDeltaTime);
        }

        if (currentAnimationData == null || currentAnimationData.clip == null) return;

        deltaDisplacement += animator.deltaPosition;
        currentAnimationData.secondsPlayed += FPMath.Abs(MainScript.fixedDeltaTime * GetSpeed());
        ...
        if (currentAnimationData.secondsPlayed == currentAnimationData.length)
        {
            if (currentAnimationData.clip.wrapMode == WrapMode.Loop ||
                currentAnimationData.clip.wrapMode == WrapMode.PingPong)
            {
    ...
    private void _playAnimation(MecanimAnimationData targetAnimationData, Fix64 blendingTime, Fix64 normalizedTime,
        bool mirror)
    {
    ...
    public bool IsPlaying(MecanimAnimationData animData, float weight = 1)
    {
        ...
        if (currentAnimationData == animData && animData.wrapMode == WrapMode.Once &&
            animData.timesPlayed > 0) return false;
        if (currentAnimationData == animData && animData.wrapMode == WrapMode.Clamp &&
            animData.timesPlayed > 0) return false;
    ...
    public Vector3 GetDeltaPosition()
    {
        if ((MainScript.isConnected || MainScript.config.debugOptions.emulateNetwork)
            && MainScript.config.networkOptions.disableRootMotion) return new Vector3();
