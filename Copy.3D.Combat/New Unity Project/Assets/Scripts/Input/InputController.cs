...
public class InputController : AbstractInputController
{
    ...
    protected virtual void SelectInputType()
    {
        ...
        if (this.getAxis == null)
        {
            ...
            if (MainScript.isCInputInstalled &&
                MainScript.config.inputOptions.inputManagerType == InputManagerType.cInput)
            {
    ...
    protected virtual void InitializeCInput()
    {
        ...
        Type inputType = MainScript.SearchClass("cInput");
        ...
        if (inputType != null)
        {
            ...
            if (getAxisInfo != null)
            {
                this.getAxis = delegate(string axis) { return (float) getAxisInfo.Invoke(null, new object[] {axis}); };
            }
            ...
            allowDuplicatesInfo.SetValue(
                null,
                Convert.ChangeType(MainScript.config.inputOptions.cInputAllowDuplicates,
                    allowDuplicatesInfo.PropertyType),
                null
            );

            inputType.GetField("gravity").SetValue(null, MainScript.config.inputOptions.cInputGravity);
            inputType.GetField("sensitivity").SetValue(null, MainScript.config.inputOptions.cInputSensitivity);
            inputType.GetField("deadzone").SetValue(null, MainScript.config.inputOptions.cInputDeadZone);
