...
public class ControlsScript : MonoBehaviour {
    ...
    void Start() {
		...
		if (gameObject.name == "Player1") {
            ...
            if (MainScript.gameMode == GameMode.TrainingRoom) {
                myInfo.currentLifePoints = (Fix64)myInfo.lifePoints * (MainScript.config.trainingModeOptions.p1StartingLife / 100);
                myInfo.currentGaugePoints = (Fix64)myInfo.maxGaugePoints * (MainScript.config.trainingModeOptions.p1StartingGauge / 100);
            } else {
            ...
            if (MainScript.gameMode == GameMode.TrainingRoom) {
                myInfo.currentLifePoints = (Fix64)myInfo.lifePoints * (MainScript.config.trainingModeOptions.p2StartingLife / 100);
                myInfo.currentGaugePoints = (Fix64)myInfo.maxGaugePoints * (MainScript.config.trainingModeOptions.p2StartingGauge / 100);
            } else {
	...
	private void validateRotation(){
		...
		if (currentSubState == SubStates.Stunned && !MainScript.config.characterRotationOptions.fixRotationWhenStunned) return;
		if (isBlocking && !MainScript.config.characterRotationOptions.fixRotationWhenBlocking) return;
		if (MainScript.config.characterRotationOptions.rotateOnMoveOnly && myMoveSetScript.IsAnimationPlaying("idle")) return;
	...
	public void DoFixedUpdate(
        ...
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            ...
            || (playerNum == 2 && MainScript.config.trainingModeOptions.p2Gauge == LifeBarTrainingMode.Infinite))) RefillGauge();

        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && myInfo.currentLifePoints < myInfo.lifePoints 
            && ((playerNum == 1 && MainScript.config.trainingModeOptions.p1Life == LifeBarTrainingMode.Infinite) 
            || (playerNum == 2 && MainScript.config.trainingModeOptions.p2Life == LifeBarTrainingMode.Infinite))) RefillLife();
		...
		foreach (InputReferences inputRef in currentInputs.Keys){
			...
			if (inputHeldDown[inputRef.engineRelatedButton] > 0 && inputHeldDown[inputRef.engineRelatedButton] <= (2 / (Fix64)MainScript.config.fps)){
        ...
        if (applyRootMotion || (currentMove != null && currentMove.applyRootMotion))
        {
            ...
            if (myMoveSetScript.animationPaused) {
                newPosition.x += myHitBoxesScript.GetDeltaPosition().x * myMoveSetScript.GetAnimationSpeed() * MainScript.timeScale;
                newPosition.y += myHitBoxesScript.GetDeltaPosition().y * myMoveSetScript.GetAnimationSpeed() * MainScript.timeScale;
            } else {
        ...
        normalizedDistance = FPMath.Clamp(FPVector.Distance(opControlsScript.worldTransform.position, worldTransform.position) / MainScript.config.cameraOptions._maxDistance, 0, 1);
            ...
            if (currentMove != null && MainScript.config.executionBufferType == ExecutionBufferType.OnlyMoveLinks) {
                foreach (FrameLink frameLink in currentMove.frameLinks) {
                    ...
                    if (confirmQueue) {
                      ...
                      || (currentMove == null
                          && storedMoveTime >= ((Fix64)(MainScript.config.executionBufferTime - 2) / (Fix64)MainScript.config.fps))) {
				confirmQueue = true;
			}
	...
	private void translateInputs(
		...
		foreach (InputReferences inputRef in currentInputs.Keys) {
			...
			if (inputRef.inputType != InputType.Button && inputHeldDown[inputRef.engineRelatedButton] > 0 && ev.axisRaw == 0) {
				if ((inputRef.engineRelatedButton == ButtonPress.Back && MainScript.config.blockOptions.blockType == BlockType.HoldBack)){
					potentialBlock = false;
				}
			...
			if (inputRef.inputType != InputType.Button && ev.axisRaw != 0) {
				if (inputRef.inputType == InputType.HorizontalAxis) {
					...
					if (ev.axisRaw > 0) {
						...
						if (inputHeldDown[inputRef.engineRelatedButton] == MainScript.fixedDeltaTime && testMoveExecution(inputRef.engineRelatedButton, false)) return;
					...
					if (((inputRef.engineRelatedButton == ButtonPress.Back && MainScript.config.blockOptions.parryType == ParryType.TapBack) ||
					     (inputRef.engineRelatedButton == ButtonPress.Forward && MainScript.config.blockOptions.parryType == ParryType.TapForward))
			...
			if (inputRef.inputType == InputType.Button && !MainScript.config.lockInputs){
				...
				if (ev.button) {
                    ...
                    // Plinking
					if (inputHeldDown[inputRef.engineRelatedButton] <= ((Fix64)MainScript.config.plinkingDelay/(Fix64)MainScript.config.fps)) {
						foreach (InputReferences inputRef2 in currentInputs.Keys) {
							InputEvents ev2 = currentInputs[inputRef2];
							InputEvents p2;
							if (!previousInputs.TryGetValue(inputRef2, out p2)){
							...
							if (inputRef2 != inputRef && inputRef2.inputType == InputType.Button && button2Down) {
                                ...
                                if (tempMove != null){
                                    if (currentMove != null && currentMove.currentFrame <= MainScript.config.plinkingDelay) KillCurrentMove();
                                    storedMove = tempMove;
									storedMoveTime = ((Fix64)MainScript.config.executionBufferTime / (Fix64)MainScript.config.fps);
									return;
								}
	...
	public void ReadMove(MoveInfo move){
		...
		foreach (Projectile projectile in move.projectiles){
			if(
                ...
                GameObject pTemp = MainScript.SpawnGameObject(projectile.projectilePrefab, newPos.ToVector(), Quaternion.identity, durationFrames);
		...
		foreach (MoveParticleEffect particleEffect in move.particleEffects){
			if (
                ...
                long frames = particleEffect.particleEffect.destroyOnMoveOver? (move.totalFrames - move.currentFrame) : Mathf.RoundToInt(particleEffect.particleEffect.duration * MainScript.config.fps);
                ...
                GameObject pTemp = MainScript.SpawnGameObject(particleEffect.particleEffect.prefab, Vector3.zero, newRotation, frames);
		...
		foreach (CameraMovement cameraMovement in move.cameraMovements){
			...
			if (cameraMovement.casted && !cameraMovement.over && cameraMovement.time >= cameraMovement._duration && MainScript.freeCamera){
			...
			if (move.currentFrame >= cameraMovement.castingFrame){
				...
				if (cameraMovement.cinematicType == CinematicType.CameraEditor){
                    ...
                    emulatedCam = MainScript.SpawnGameObject(cameraMovement.prefab, transform.position, Quaternion.identity);
                ...
                move.currentFrame <= hit.activeFramesEnds)
            {
                if (hit.hurtBoxes.Length > 0)
                {
                    ...
                    { // HURTBOX TEST
                        ...
                        if ((opControlsScript.worldTransform.position.x >= MainScript.config.selectedStage._rightBoundary - 2 ||
                             opControlsScript.worldTransform.position.x <= MainScript.config.selectedStage._leftBoundary + 2)
                            && myPhysicsScript.IsGrounded()
	...
	private void ReleaseStun(
		...
		if (!isBlocking && comboHits > 1 && MainScript.config.comboOptions.comboDisplayMode == ComboDisplayMode.ShowAfterComboExecution){
	...
	public void GetHitParry(Hit hit, int remainingFrames, FPVector[] location){
		...
		if (MainScript.config.blockOptions.parryStunType == ParryStunType.Fixed){
			...
			if (hit.hitStunType == HitStunType.FrameAdvantage) {
				...
				stunFrames = (int)FPMath.Round(((Fix64)(stunFrames * MainScript.config.blockOptions.parryStunFrames)/(Fix64)100));
				if (stunFrames < 1) stunFrames = 1;
				stunTime = (Fix64)stunFrames/(Fix64)MainScript.config.fps;
			}else{
		...
		if (location.Length > 0 && particle != null){
            ...
            GameObject pTemp = MainScript.SpawnGameObject(particle, GetParticleSpawnPoint(spawnPoint, location), Quaternion.identity, frames);
        ...
        if (MainScript.config.blockOptions.parryHitEffects.autoHitStop) {
            MainScript.DelaySynchronizedAction(myHitBoxesScript.ResetHit, GetHitFreezingTime(hit.hitStrength) * spaceBetweenHits);
        } else {
            MainScript.DelaySynchronizedAction(myHitBoxesScript.ResetHit, MainScript.config.blockOptions.parryHitEffects._hitStop * spaceBetweenHits);
        }
	...
	public void GetHit(Hit hit, int remainingFrames, FPVector[] location, bool ignoreDirection = false){
					...
					if (frameLink.linkType == LinkType.CounterMove){
                        ...
                        if (cancelable) {
							...
							if (frameLink.disableHitImpact) {
                                Fix64 timeLeft = (Fix64)(currentMove.totalFrames - currentMove.currentFrame)/(Fix64)MainScript.config.fps;
        ...
        if (!hit.armorBreaker && currentMove != null &&
			...
			if ((MainScript.config.counterHitOptions.startUpFrames && currentMove.currentFrameData == CurrentFrameData.StartupFrames) ||
			    (MainScript.config.counterHitOptions.activeFrames && currentMove.currentFrameData == CurrentFrameData.ActiveFrames) ||
			    (MainScript.config.counterHitOptions.recoveryFrames && currentMove.currentFrameData == CurrentFrameData.RecoveryFrames)){
		...
		if (location.Length > 0 && hitEffects.hitParticle != null){
            ...
            GameObject pTemp = MainScript.SpawnGameObject(hitEffects.hitParticle, newLocation, Quaternion.identity, frames);
		...
		if (comboHits > 1 && MainScript.config.comboOptions.comboDisplayMode == ComboDisplayMode.ShowDuringComboExecution){
			MainScript.FireAlert(MainScript.config.selectedLanguage.combo, opInfo);
		}
		...
		if ((currentMove == null || !currentMove.hitAnimationOverride) && (!armored || isDead)) {
			...
			if (hit.hitStunType == HitStunType.FrameAdvantage) {
				...
				if (stunFrames < 1) stunFrames = 1;
				if (stunFrames < MainScript.config.comboOptions._minHitStun) stunFrames = MainScript.config.comboOptions._minHitStun;
				stunTime = (Fix64)stunFrames/(Fix64)MainScript.config.fps;
			}else if (hit.hitStunType == HitStunType.Frames) {
            ...
            if (pushForce.y > 0 || (isDead && !isKnockDown)) {
                ...
                if (pushForce.y < MainScript.config.comboOptions._minPushForce) pushForce.y = MainScript.config.comboOptions._minPushForce;
				airJuggleHits ++;
			}
            ...
            if ((airHit || (!myPhysicsScript.IsGrounded() && airRecoveryType == AirRecoveryType.DontRecover))
				...
				if (hit.hitType == HitType.HighKnockdown){
                    ...
                    if (!hit.customStunValues) stunTime =
                        MainScript.config.knockDownOptions.high._knockedOutTime + MainScript.config.knockDownOptions.high._standUpTime;
                    ...
                    airRecoveryType = AirRecoveryType.DontRecover;
                    if (!hit.customStunValues) stunTime =
                        MainScript.config.knockDownOptions.highLow._knockedOutTime + MainScript.config.knockDownOptions.highLow._standUpTime;
                    ...
                    airRecoveryType = AirRecoveryType.DontRecover;
                    if (!hit.customStunValues) stunTime =
                        MainScript.config.knockDownOptions.sweep._knockedOutTime + MainScript.config.knockDownOptions.sweep._standUpTime;
    ...
    private void RemoveGauge(Fix64 gaugeLoss) {
        ...
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && ((playerNum == 1 && MainScript.config.trainingModeOptions.p1Gauge == LifeBarTrainingMode.Refill)
            || (playerNum == 2 && MainScript.config.trainingModeOptions.p2Gauge == LifeBarTrainingMode.Refill))) {
                if (!MainScript.FindAndUpdateDelaySynchronizedAction(this.RefillGauge, MainScript.config.trainingModeOptions.refillTime)) 
				MainScript.DelaySynchronizedAction(this.RefillGauge, MainScript.config.trainingModeOptions.refillTime);
		}
	}
	...
	private bool DamageMe(Fix64 damage){
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && playerNum == 1 && MainScript.config.trainingModeOptions.p1Life == LifeBarTrainingMode.Infinite) return false;
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && playerNum == 2 && MainScript.config.trainingModeOptions.p2Life == LifeBarTrainingMode.Infinite) return false;
        ...
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            ...
            || (playerNum == 2 && MainScript.config.trainingModeOptions.p2Life == LifeBarTrainingMode.Refill))) {
                ...
                if (!MainScript.FindAndUpdateDelaySynchronizedAction(this.RefillLife, MainScript.config.trainingModeOptions.refillTime)) {
                    MainScript.DelaySynchronizedAction(this.RefillLife, MainScript.config.trainingModeOptions.refillTime);
                }
		}

        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && playerNum == 1 && MainScript.config.trainingModeOptions.p1Life != LifeBarTrainingMode.Normal) return false;
        if ((MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)
            && playerNum == 2 && MainScript.config.trainingModeOptions.p2Life != LifeBarTrainingMode.Normal) return false;
	...
	public void ResetData(bool resetLife){
		if (MainScript.config.roundOptions.resetPositions){
			if (playerNum == 1){
				worldTransform.position = new FPVector(MainScript.config.roundOptions._p1XPosition, .009, worldTransform.position.z);
			}else{
                worldTransform.position = new FPVector(MainScript.config.roundOptions._p2XPosition, .009, worldTransform.position.z);
			}
		...
		if (resetLife || MainScript.config.roundOptions.resetLifePoints){
            if (playerNum == 1 && (MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)) {
                myInfo.currentLifePoints = (Fix64)myInfo.lifePoints * (MainScript.config.trainingModeOptions.p1StartingLife / 100);
            } else if (playerNum == 2 && (MainScript.gameMode == GameMode.TrainingRoom || MainScript.gameMode == GameMode.ChallengeMode)) {
                myInfo.currentLifePoints = (Fix64)myInfo.lifePoints * (MainScript.config.trainingModeOptions.p2StartingLife / 100);
            } else {
                myInfo.currentLifePoints = (Fix64)myInfo.lifePoints;
            }
